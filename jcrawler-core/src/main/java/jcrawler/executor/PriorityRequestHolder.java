...
public class PriorityRequestHolder implements RequestHolder {
	
	private Logger logger = LoggerFactory.getLogger(PriorityRequestHolder.class);
	
	private Reserver reserver;
	
	private BlockingQueue<Request> requestQueue = new PriorityBlockingQueue<Request>(
			Envirenment.DEFAULT_REQUEST_THRESHOLDS, new Comparator<Request>() {

				@Override
				public int compare(Request arg0, Request arg1) {
					return arg0.prior() - arg1.prior();
				}
	 ...
	 * @param reserver
	 */
	public PriorityRequestHolder(Reserver reserver) {
	...
	public boolean push(Request request) {
		...
		if (request == null || !request.validate()) {
			logger.warn("the input request {} is invalid and couldn't be pushed to PriorityRequestHolder!", request);
			return false;
		}
		// 验证reserver策略
